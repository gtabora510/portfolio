...
1. Developer asks Copilot: "Get Page Contents" in the IDE.
2. Copilot's MCP client routes the request to the approved Confluence MCP server.
3. Confluence MCP calls the Confluence APIs, applies org policies, and returns structured data to Copilot.
...
| --------------------------------- | ------------------------ | ------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| GitHub Copilot Onboarding         | Required                 | Complete both the onboarding and installation of the GitHub Copilot IDE plugin. | Follow the instruction in Onboarding to GitHub Copilot and Installing the Plugin. |
...
4. Locate `confluence-mcp-server` in the list of MCP servers and click **Install**.
5. Return to the chat window and enter Agent mode.
6. Type an example query such as "What are the Confluence pages I recently worked on?" to initiate authentication. A pop-up displays: the MCP Server Definition `jira.mcp.prod.aws.acme.net/mcp` wants to authenticate to `jira.mcp.prod.aws.acme.net`.
...
5. Install the plugin:
   ...
   3. Click **Install** under **Confluence MCP Server**.
   4. Click **Allow** when "The MCP Server Definition '110128-confluence-mcp-server' wants to authenticate to `ide.acme.com`" prompt appears.
